...
namespace Syrus.Plugins.DFV2Client
{
    ...
    public class DialogFlowV2Client : MonoBehaviour
    {
        ...
        /// </summary>
        public ServiceSettings[] accessSettingsArr;
        public int index = -1;
        ...
        public void DetectIntentFromText(string text, string talker, string languageCode = "")
        {
            if (languageCode.Length == 0)
                languageCode = accessSettingsArr[index].LanguageCode;
        ...
        private IEnumerator DetectIntent(DF2QueryInput queryInput, string session)
        {
            ...
            while (!JwtCache.TryGetToken(accessSettingsArr[index].ServiceAccount, out accessToken))
                yield return JwtCache.GetToken(accessSettingsArr[index].CredentialsFileName,
                    accessSettingsArr[index].ServiceAccount);
            ...
            string url = string.Format(PARAMETRIC_DETECT_INTENT_URL, accessSettingsArr[index].ProjectId, session);
            ...
            else
            {
                ...
                if (resp.queryResult.outputContexts != null)
                {
                    ...
                    Debug.Log("다이얼로그 플로우 문제가 생겼는데 뭔질 모르겠다");
            }
        }
        ...
        public void AddInputContext(DF2Context inputContext, string session)
        {
            if (!inputContext.Name.StartsWith("projects"))
                inputContext.Name = string.Format(DF2Context.PARAMETRIC_CONTEXT_ID,
                    accessSettingsArr[index].ProjectId, session, inputContext.Name);
        ...
        public void AddEntityType(DF2EntityType entityType, string session)
        {
            if (!entityType.Name.StartsWith("projects"))
                entityType.Name = string.Format(DF2EntityType.PARAMETRIC_ENTITY_TYPE_NAME,
                    accessSettingsArr[index].ProjectId, session, entityType.Name);
        ...
        private IEnumerator ClearSessionRequest(string session)
        {
            ...
            string url = string.Format(
                ...
                accessSettingsArr[index].ProjectId, session);
